...
import { renderStockBubbles } from '../utils/bubbleChart.js';
...
export function initStockPlayfield(container, { symbols, sp500Companies }) {
  const sp500Map = new Map(
    (sp500Companies || []).map((c) => [c.Symbol?.toUpperCase(), c])
  );
  ...
  function pushMarketsUrl(symbol) {
    window.dispatchEvent(
      new CustomEvent('marketscope:syncMarketsUrl', { detail: { tab: 'stocks', symbol } })
    );
  }
  ...
  function renderEmptyState() {
    renderStockBubbles(bubbleChartEl, {
      companies: sp500Companies,
      symbolSet: new Set((symbols || []).map((s) => String(s).toUpperCase())),
  ...
  async function loadAndRenderStock(sym) {
    ...
    if (et) et.innerHTML = '';
    try {
      const ohlcv = await DataLoader.loadUSStock(sym);
      currentData = ohlcv;
  ...
  function renderStockDetail(sym, displayData, fullData = null) {
    ...
    headerEl.innerHTML = `
      <div class="playfield-header-left">
        <div class="playfield-logo">${(meta?.Shortname || sym).charAt(0)}</div>
        <div>
          <h1 class="playfield-company-name">${meta?.Shortname || meta?.Longname || sym}</h1>
          <p class="playfield-ticker">${meta?.Exchange || 'NYSE'}: ${sym}</p>
        </div>
      </div>
    `;
          ...
          inRange.forEach((evt) => {
            ...
            if (cx >= 0 && cx <= width) {
              ...
              circle.on('click', (e) => {
                ...
                if (filtered.length) {
                  renderStockDetail(currentSymbol, filtered, currentData);
          ...
          .map(
            (c) =>
              `<button type="button" class="playfield-trending-item" data-symbol="${c.Symbol}">
                ...
                <span class="playfield-trending-name">${(c.Shortname || c.Longname || c.Symbol).slice(0, 20)}${(c.Shortname || c.Longname || '').length > 20 ? '…' : ''}</span>
              </button>`
          )
          .join('')}
      </div>
